...
.ggsci_db_igv_default <- c(
  ...
  "#5A655E", "#CC9900", "#99CC00", "#A9A9A9", "#33CC00", "#00CC33", "#00CC99",
  "#0099CC", "#0A47FF", "#4775FF", "#FFC20A", "#FFD147", "#990033", "#991A00",
  "#996600", "#809900", "#339900", "#00991A", "#009966", "#008099", "#003399",
  "#1A0099", "#660099", "#990080", "#D60047", "#FF1463", "#00D68F", "#14FFB1"
)


#' Generate character vector of unique color hex codes
#' Base color palette adopts \code{ggsci:::ggsci_db$igv$default}
#' @param n Number of unique colors to generate. When not exceeding existing
#' number of colors, just return the first \code{n} colors, otherwise generate
#' interpolated colors.
uniqueColors <- function(n) {
  if (n <= length(.ggsci_db_igv_default)) {
    .ggsci_db_igv_default[seq_len(n)]
  } else {
    warning("More than ", length(.ggsci_db_igv_default), " unique colors ",
            "requested. Interpolating preset color set.")
    grDevices:::colorRampPalette(.ggsci_db_igv_default)(n)
  }
}

...
We use Slide-tags snRNA-seq data on mouse embryonic E14 brain for this tutorial. It is publicly available on Single-Cell Portal via [SCP2170](https://singlecell.broadinstitute.org/single_cell/study/SCP2170/slide-tags-snrna-seq-on-mouse-embryonic-e14-brain), originally produced in the work of [Andrew J. C. Russell et al., 2023](https://doi.org/10.1101/2023.04.01.535228). We provide the following pre-cleaned data that can be easily loaded into user R environment at [figshare](https://figshare.com/articles/dataset/CytoSignal_example_dataset/24938868). 
...
We prepared our database based on CellphoneDB V2. The database is delivered together with the CytoSignal package and simply running the command below can load it into the object. 
...
The function `suggestInterval()` recommends the top five closest distances between cells. For stereo-seq data, the interval between the two neighboring should be exactly 1. If the top few intervals suggested are not 1, then there might be something wrong in the preprocessing step.
...
TEXT THAT EXPLAINS WHY USING GAUSSIAN KERNEL FOR DIFFUSIBLE LIGANDS.

Before imputing, we use the function `inferEpsParams()` to infer the epsilon parameter for the Gaussian kernel, which will be used later for imputing the expression of diffusible ligands.

```{r inferEps, results}
cs <- inferEpsParams(cs, tech.res = 0.5, bin_size = 50, loc.d = 1, thresh = 1e-9)
```

Optionally, the inferred parameters can be accessed as shown below.
...
With the parameters inferred, we can then find the nearest neighbors within a 200 um radius of each cell, together with the weights for each neighbor calculated through Gaussian kernel.
...
# Normalize the imputed expression. Defualt normalization method is 10000*log1p(x)
cs <- normCounts(cs, "default")
```

...
With function `inferIntrDEG()`, we can identify genes that are differentially expressed in the cells that are highly correlated to an interaction. After obtaining a gene set for each interaction, we further fit it to a [glmnet](https://glmnet.stanford.edu/articles/glmnet.html) model to do a regression analysis. We finally examine the coefficients of the model to determine the significant genes for the interaction.
...
With the gene set, we can further do a GO term enrichment analysis. Technically this can be done with any GO tools of preference. We used [GORILLA](https://cbl-gorilla.cs.technion.ac.il/) in our manuscript. Here we provide an example of using the R package [gprofiler2](https://CRAN.R-project.org/package=gprofiler2).
...
In our manuscript, we used another web-based tool [REVIGO](http://revigo.irb.hr/) to visualize the GO terms. Users can simply copy the term IDs from the result into the web form and view the visualization interactively in browser. For users with limited access to interactive session (e.g. on HPCs), we provided an out-of-package script that can do the query and visualize the result from R end. The script is available at ...TBD.
